...
export function OnboardingChatWidget({ currentStep }: OnboardingChatWidgetProps) {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      const welcomeMessage: Message = {
        id: 'welcome',
        role: 'assistant',
        content: `Hello! Need help signing up?\n\nI speak Portuguese, Spanish, English, and more. Just ask me anything!\n\n---\n\nOla! Precisa de ajuda? Falo portugues!\n\n---\n\nHola! Necesitas ayuda? Hablo espanol!`,
        timestamp: new Date(),
      }
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 bg-[#C4785A] text-white rounded-full shadow-lg transition-all hover:bg-[#B56A4F] active:scale-95 ${
          isOpen ? 'hidden' : 'flex'
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/30">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] flex flex-col overflow-hidden">
            {/* Header */}
            <div className="bg-[#C4785A] text-white px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
                  <span className="text-lg">?</span>
                </div>
                <div>
                  <h3 className="font-semibold">Signup Help</h3>
                  <p className="text-xs text-white/80">PT / ES / EN / DE / FR</p>
                </div>
              </div>
              <button
            ...
            <div className="p-3 border-t border-[#EBEBEB] bg-white">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl border border-[#DEDEDE] focus:outline-none focus:border-[#C4785A] text-sm"
                />
                <button
                  onClick={handleSend}
                  disabled={!input.trim() || sending}
                  className="px-4 py-2.5 bg-[#C4785A] text-white rounded-xl font-medium disabled:opacity-50 disabled:cursor-not-allowed text-sm hover:bg-[#B56A4F]"
                >
                  Send
                </button>
              </div>
